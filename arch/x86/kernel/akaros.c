...
/* Helpful one-liner:
pr_info("XME %d %s %s %d\n", smp_processor_id(), __FILE__, __func__, __LINE__);
 ...
 * - I've occasionally noticed clocksource_watchdog() complain about the TSC and
 *   think it is unstable.
 * - We could paravirt TSC_DEADLINE_TIMER.
 * - Could mess with initrd compression mechanisms for faster unzip
 * - Could look into CPU_IDLE_GOV_HALTPOLL and PVPANIC
 */
static void __init akaros_init_platform(void)
{
	/* Ignored in the kernel (emsr_fakewrite()). */
	setup_clear_cpu_cap(X86_FEATURE_TSC_DEADLINE_TIMER);
	/* Doubt we support this, usually don't even build it into Linux. */
	setup_clear_cpu_cap(X86_FEATURE_MTRR);
	/* VM_ENTRY_LOAD_IA32_PAT is 0: we don't save/restore guest PAT */
	setup_clear_cpu_cap(X86_FEATURE_PAT);
	/* intel thermal msrs */
...
static void __init akaros_guest_late_init(void)
{
	size_t sz = real_mode_size_needed();
	phys_addr_t mem;

	if (sz) {
		/*
		 * Akaros doesn't support real mode, but we still need a
		 * trampoline.  We don't have AP code executing in real-mode
		 * either.  Put it anywhere...
		 */
		mem = memblock_phys_alloc(sz, PAGE_SIZE);
		if (!mem)
			pr_err("Couldn't get mem for real_mode_trampoline\n");
		else
...
const __initconst struct hypervisor_x86 x86_hyper_akaros = {
	...
	.init.guest_late_init	= akaros_guest_late_init,
	.ignore_nopv		= true,
};
